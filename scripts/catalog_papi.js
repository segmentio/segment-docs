...
const getConnectionModes = (destination) => {
  ...
  if (destination.components.length == 0) {
    switch (true) {
      case destination.platforms.mobile:
        connectionModes.device.mobile = true
      case destination.platforms.browser:
        if (destination.browserUnbundling) {
          connectionModes.cloud.web = true
        }
        connectionModes.device.web = true
      case destination.platforms.server:
...
const getConnectionModesNew = (destination) => {
  let connectionModesNew = {
    device: {
      web: false,
      mobile: false,
      server: false
  ...
  switch (true) {
    case destination.platforms.mobile:
      connectionModesNew.device.mobile = true
    case destination.platforms.browser:
      if (destination.browserUnbundling) {
        connectionModesNew.cloud.web = true
      }
      connectionModesNew.device.web = true
    case destination.platforms.server:
      connectionModesNew.cloud.mobile = true
      connectionModesNew.cloud.server = true
      if (destination.platforms.browser) {
        connectionModesNew.cloud.web = true
      }
  }
