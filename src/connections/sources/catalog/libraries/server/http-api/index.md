...
Common reasons events are not accepted by Segment include: 
  ...
  - **The `\batch` API endpoint** This endpoint accepts a maximum of 500KB per 1 batched request, up to 2500 batched events in the request, and 32KB per each event. Segment will return a `200` response but reject the event, when the number of batched events exceeds limit.
