...
Segment Webhooks uses internet protocol and HTTP callback to submit real-time user data to your own HTTP endpoints. A Webhook is an HTTP callback: a simple event-notification using HTTP POST. A web application implementing Webhooks will POST a message to a URL when certain things happen. 
...
2. Click on **Destinations** on the Catalog page. 
3. Search for **Webhooks** in the Destinations Catalog, and select the destination.
4. Click **Configure Webhooks**.
5. Select the web source that will send data to Webhooks (Actions) and follow the steps to name your destination. 
6. On the **Settings** tab, specify up to five different Webhook URLs, you would like to forward data to.
7. If you require authentication, add in a shared secret.
8. Once enabled, Segment sends data to the configured webhook.
