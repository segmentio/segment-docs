...
Before you connect Segment to Google Analytics 4, configure a Google Analytics 4 property in your Analytics account. Additionally, be sure to plan and enable any Custom Definitions in your GA4 Admin Panel prior to sending data from Segment to GA4.  For more information, see Google's article [Set up Analytics for a website and/or app](https://support.google.com/analytics/answer/9304153){:target='_blank'}.
...
4. Select the web source that will send data to Google Analytics 4 and follow the steps to name your destination. The web source chosen must use [Analytics.js 2.0](/docs/connections/sources/catalog/libraries/website/javascript/).  For mobile source tracking, view Firebase Destination configuration {{insert link}}.
...
8.  In order for GA4 to accept events on page, enable Set Configuration Mapping triggered by the first Segment event called after analytics.load().  Set Configuration Mapping calls the gtag(‘config’) command to enable tracking to your GA4 Measurement ID.  This command MUST be called at each new page load.
...
Some events automatically populate in Google Analytics. For example, there are some  [Automatically Collected] (https://support.google.com/analytics/answer/9234069){:target=”_blank”} events collected by triggering the Set Configuration Mapping (i.e. calling gtag(‘config’), and [Enhanced Measurement  events](https://support.google.com/analytics/answer/9216061){:target=”_blank”} controlled by toggling “on” in your GA4 Admin panel use event listeners to send events; in addition, all events tracked via GA4 Web, populate some commonly used parameters like page_location without additional configuration.  Review Google’s documents for more information found [here](https://support.google.com/analytics/table/13594742){:target=”_blank”}  
...
Google Analytics 4 does not accept custom event names that include spaces. Segment replaces spaces in the Event Name in the Custom Event action with an underscore. As a result, you will see custom events as snake_case  in Google Analytics 4.
...
Keep in mind that Custom Events will not appear in some of Google's standard reports; you will need to set up custom reports for meaningful analysis. 

> info "Custom Events with Item Parameters"
> To pass item parameters, you must use Google Recommended Event names that accept the items array, in other words, Ecommerce Events.  Item parameters will not pass to GA4 with a Custom Event name or any other Event name if the items array is not specified as a parameter.
...
Automatically Collected and Enhanced Measurement events include a defined set of parameters by default. Google also provides a set of required and optional parameters to include with each Recommended event. Additionally, you can add more event parameters when you need them.  It is advised to review GA4’s list of defined event parameters - anything beyond that list is a custom event parameter.  Lastly, be knowledgeable of the limits of how many Custom Definitions your GA4 instance allows as well as how many parameters you can send with each event, [GA4 Limits](https://https://support.google.com/analytics/answer/9267744){:target=_blank”}
...
3. Click the toggle on for **Enable Consent Mode**, this will call gtag(‘consent’,’default’) with the defined parameters when the gtag library loads.
