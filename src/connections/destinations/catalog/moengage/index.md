...
Once the Segment library is integrated with your app, toggle MoEngage on in your Segment integrations. These new settings will take up to an hour to propagate to all of your existing users. For new users, it'll be instantaneous!

Segment-MoEngage Integration is a bundled integration, meaning it requires a client-side integration.

## Setup MoEngage in Segment Dashboard

  1. Get your key ([AppID](http://app.moengage.com/v3/#/settings/0/0)) from MoEngage dashboard.
  ...
  5. Save the changes.
  6. Make sure the `Connection Mode` is set to `Device-mode`. This is required in order to make use of features like push notification and other in-app features of MoEngage SDK.

These new settings will take up to an hour to propagate to all of your existing users. For new users, it'll be instantaneous! Segment-MoEngage Integration is a bundled integration, requires client side integration.
...
To get started with MoEngage on iOS, first integrate your app with the [MoEngage-Segment-iOS](https://github.com/moengage/MoEngage-Segment-iOS) library. MoEngage can be integrated via Segment using [CocoaPods](http://cocoapods.org).
  ...
  * Initialise pod with `pod init`; this will create a podfile for your project.
...
Event tracking is used to track user behaviour in an app. `track` lets you record the actions your users perform. Every action triggers an "event", which can also have associated attributes.
...
**IMPORTANT**:
To sign up to MoEngage's EU environment go to the following [URL](https://app-eu.moengage.com).
...
#### Segment Docs:
For more information on using **Segment for iOS**, refer to [**Developer Docs**](https://segment.com/docs/connections/sources/catalog/libraries/mobile/ios/) provided by Segment.

## Android

To get up and running with MoEngage on Android, there a couple of steps we will walk you through.
To enable MoEngage's Android SDK's full functionality (like Push Notifications, InApp Messaging), there are still a couple of steps that you have to take care of in your Android app.
...
This is only required for migrations to MoEngage Platform so the SDK can determine whether the user is a new user for on your app or an existing user who has updated to the latest version.
...
Copy the [Server Key](https://docs.moengage.com/docs/getting-fcmgcm-server-key) from the FCM console and add it to the MoEngage Dashboard. To upload, go to Settings and add the Server Key and package name. Please make sure you add the keys in both your Test and Live environments.

#### Adding meta information for push notification.

Add the AppId, the smalll and large notification icons to the builder:
...
The Application needs to pass the Push Token received from FCM to the MoEngage SDK for the MoEngage platform to send out push notifications to the device. Use the below call to pass the push token to the MoEngage SDK.

```java
PushManager.getInstance().refreshToken(getApplicationContext(), token);
```
*Note*: Ensure token is passed to MoEngage SDK whenever the push token is refreshed and on application update. Passing the token on application update is important for migration to the MoEngage Platform.

#### Pass the Push payload to the MoEngage SDK

To pass the push payload to the MoEngage SDK, call the MoEngage API from onMessageReceived() from the Firebase receiver. Before passing the payload to the MoEngage's SDK, you should check if the payload is from the MoEngage platform using the helper API provided by the SDK.
...
```
When MoEngage's SDK handles push registration, it optionally provides a callback to the application whenever a new token is registered or the token is refreshed. Applications can get this callback by implementing PushManager.OnTokenReceivedListener and registering for a callback in the Application class' onCreate() using PushManager.getInstance().setTokenObserver().

#### Configure Geo-fence

By default, SDK does not track location neither geo-fence campaigns work by default. To track location and run geo-fence campaigns you need to opt-in for location service in the MoEngage initializer. To initialize call the below opt-in API.
...
*Note*: Your Application must have location permission and Play Services' Location Library should be included.

For more details on refer to the [Configuring Geo-Fence section](https://docs.moengage.com/docs/push-configuration#section-geofence-push) in the MoEngage documentation.
...
For HTTPS Web Push to work, you need to host two files in the `root` directory of your web server. This file will be available for you to download at the [web settings page](https://app.moengage.com/v3/#/settings/push/web).
...
#### Use your serviceworker file (HTTPS only)

If you already have an SW file, just add the following line to the top of your `serviceworker.js` file:
