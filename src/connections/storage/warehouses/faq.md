...
and the property refers to the data being collected (user_id). Note that it is not possible to have different sources feed data into the same schema in your warehouse. In fact, you will not have an option to use a schema name that is associated with another source while setting up a new one. 
