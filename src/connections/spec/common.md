...
If you are using the Segment server Source libraries, or passing calls directly to the HTTP API endpoint, you can manually set the `timestamp` field.  If you are using a Segment Source in device mode, the library generates `timestamp` and you cannot manually set one. However for mobile sources, the timestamp can be manually set by using middlewares to modify the payloads. (You might manually set or override a timestamp to import historical data to the Segment API). 
