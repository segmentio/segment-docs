...
**Note:** You might be used to some destinations recognizing special traits by slightly different names. For example, Mixpanel recognizes a `$created` trait when the user's account was first created, while Intercom recognizes the same trait as `created_at` instead. We will do our best to handle all the destination-specific conversions for you automatically. If you need help understanding if a specific field will be converted to a destination, please take a look at our [open source integration code](https://github.com/segment-integrations?q=&type=all&language=&sort=) or the specific destination documentation itself where you’ll find more information or [contact our support team](https://app.segment.com/workspaces?contact=1)
