...
Schema displays all the unique events, properties and traits sent through a Segment source. It also records when each data point was last seen, and how many times over the past 30 days.

Each source has its own Schema, except Object Cloud Sources which are not used for event/trait collection.

The Segment Spec is flexible and the Schema serves as a canonical reference for what data is currently collected and what you can expect to find in all the connected downstream destinatiosn.

Schema is often used in conjunction with the Debugger to QA changes to instrumentation.

## Clear Schema History

If the Schema has been polluted, or you need a clean start before a QA test, it is helpful to clear the schema.

Clear Schema History is found in the Source's Settings, under Schema Configuration.

Workspaces on the Business Plan can leverage the Schema to block events from being collected by Segment. While clearing the schema there are options for whether to retain the active blocking.

---

**The following features are available om the Business Plan**
...
Please refer to [Filtering your Segment Data](https://segment.com/docs/guides/filtering-data/) for exhaustive list of filtering capabilities.
