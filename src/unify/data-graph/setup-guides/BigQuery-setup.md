...
**Segment requires write access to this dataset for internal bookkeeping and to store checkpoint tables for the queries that are executed. Therefore, Segment recommends creating a new dataset for this purpose.** This is also the dataset you'll be required to specify when connecting BigQuery with the Segment app.
