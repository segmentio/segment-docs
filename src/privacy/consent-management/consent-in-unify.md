...
In addition to enforcing consent in Connections, you might need to have these preferences flow to each destination. By sending consent preferences downstream to your destinations, the destinations can be aware when an end-user revokes their consent. You can use the [Destination Actions framework](/docs/connections/destinations/destination-actions) to edit the destination's mapping and copy the consent preferences from the Segment Consent Preference Updated event sent from any of your sources to a destination-specified consent field. 
